...
class FeatureConverter {
  ...
  FeatureConverter(Map<String, int> vocab, bool doLowerCase, this.maxSeqLen) {
    tokenizer = FullTokenizer(vocab, doLowerCase);
  }

  Feature convert(String text) {
    // 1) tokenize
    List<String> tokens = tokenizer.tokenize(text);

    // 2) truncate
    if (tokens.length > maxSeqLen - 2) {
      tokens = tokens.sublist(0, maxSeqLen - 2);
    }

    // 3) [CLS] + tokens + [SEP]
    List<String> wordPieces = ["[CLS]", ...tokens, "[SEP]"];

    // 4) token → id
    List<int> inputIds = tokenizer.convertTokensToIds(wordPieces);

    
    List<int> inputMask = List.generate(inputIds.length, (_) => 1);
    List<int> segmentIds = List.generate(inputIds.length, (_) => 0);

    // 5) padding → length = maxSeqLen (32)
